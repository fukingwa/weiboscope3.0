...
## RT href
	...
	if (is.na(rt_href)){
		...
		retweeted_status <- mid2id(strsplit(rt_href,"/|\\?")[[1]][3])
	}
## RT text
...
check_censored <- function(id){
	token <- sample(c("82966982", "2612767607"),1)
...
InsertDB <- function(df){
  ...
  con <- dbConnect(dbDriver("PostgreSQL"), user="kwfu", password="ccfchung1206", dbname="db2019", host="147.8.144.20")
...
cont <- readline("1) Login Weibo 2) Press Return to continue \n")

while (Sys.time() < strptime("2021-02-11 19:00:00","%Y-%m-%d %H:%M:%S")) {
